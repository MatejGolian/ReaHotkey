...
SelectTab(Tab, *) {
    ...
    For Ctrl In FoundStandalone.Overlay.ChildControls {
        If Ctrl.ControlType == "TabControl" {
            Ctrl.CurrentTab := Tab
            Ctrl.Focus()
            FoundStandalone.Overlay.CurrentControlID := Ctrl.ControlID
            Break
        }
    }
}
...
DublerRevertProfileButton(*) {
    Global DublerProfileLoaded, FoundStandalone
    ...
    FoundStandalone.Overlay.Label := ""
}

DublerAutoSaveProfile() {
    Global DublerProfileLoaded, FoundStandalone, StandaloneWinCriteria

    If Not FoundStandalone Is Standalone Or FoundStandalone.Overlay.Label != "Dubler 2 Profile" Or Not WinActive(StandaloneWinCriteria)
        Return
...
DublerActivatePitchStickinessButton(Button) {
    Global AutoFocusStandaloneOverlay, DublerProfileLoaded, FoundStandalone
...
DublerEnableNotesAnnouncement(*) {
    Global DublerAnnounceNotes, FoundStandalone

    DublerAnnounceNotes := True

    Local Ctrl
    Overlay := FoundStandalone.Overlay
...
DublerToggleNotesAnnouncement(*) {
    Global DublerAnnounceNotes, FoundStandalone, StandaloneWinCriteria

    If Not FoundStandalone Is Standalone Or Not FoundStandalone.Overlay.Label == "Dubler 2 Profile" Or Not WinActive(StandaloneWinCriteria)
        Return
...
DublerSpeakNotes() {
    ...
    Global DublerAnnounceNotes, DublerNoteAnnouncements, FoundStandalone, StandaloneWinCriteria

    If Not FoundStandalone Is Standalone Or Not FoundStandalone.Overlay.Label == "Dubler 2 Profile" Or Not DublerAnnounceNotes Or FoundStandalone.Overlay.ChildControls.Length <= 1 Or Not WinActive(StandaloneWinCriteria)
        Return
...
DublerDeleteTrigger(Index, *) {
    ...
    Global AutoFocusStandaloneOverlay, StandaloneWinCriteria, DublerProfileLoaded, FoundStandalone

    Criteria := StandaloneWinCriteria
