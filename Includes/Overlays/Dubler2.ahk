...
CloseOverlay(Label := "", *) {
    ...
    Global DublerOverlayPositions, DUBLER_OVERLAY_TRIGGERS, FoundStandalone

    Local NextControl, Controls

    SoundPlay(A_ScriptDir . "\Sounds\OverlayClosed.mp3")

    If Not FoundStandalone Is Standalone
        Return

    For Trigger In DUBLER_OVERLAY_TRIGGERS {
        If Trigger["Label"] == FoundStandalone.Overlay.Label And Trigger.Has("RestoreLastPosition") And Trigger["RestoreLastPosition"] == True {
            Overlay := FoundStandalone.Overlay
            DublerOverlayPositions.Set(Trigger["Label"], GetPosition(&Overlay, FoundStandalone.Overlay.CurrentControlID))
        }
    }
    ...
    FoundStandalone.Overlay := DublerCreateLoadingOverlay(AccessibilityOverlay(Label))
}
...
DublerInit(DublerInstance) {
    Global AutoFocusStandaloneOverlay, DublerProfileLoaded, DublerRestartRequired, DublerFixSettingsOverlayShown
...
DublerSync() {
    ...
    WindowID := FoundStandalone.WindowID
...
DublerSetOverlay(*) {
    ...
    Global DUBLER_OVERLAY_TRIGGERS, DublerOverlayPositions, FoundStandalone, DublerRestartRequired
    Local Detector
    ...
    For Trigger In DUBLER_OVERLAY_TRIGGERS {
        If Not FoundStandalone Is Standalone
            Return
        If FoundStandalone.Overlay.Label != "" And (Not Trigger.Has("Popup") Or Trigger["Popup"] == False) Or (FoundStandalone.Overlay.Label == Trigger["Label"] And (Trigger.Has("Popup") And Trigger["Popup"] == True))
            Continue
        ...
        If %Detector.Name%(Trigger) {
            ...
            Overlay.Focus()
            FoundStandalone.Overlay := Overlay
